...
function Cocinero() {
  ...
  const cambiarEstado = async (id, estado) => {
    try {
      await axios.put(
        `https://pa-integral-backend.onrender.com/api/pedidos/${id}`,
        { estado }
      );

      socket.emit("estadoPedido", { id, estado });
      cargarPedidos();
    } catch (error) {
  ...
  return (
    <div className="cocinero-container">
      <h1 className="cocinero-title">Panel del Cocinero</h1>
        ...
        {pedidos.map((pedido) => (
          <div className="pedido-card" key={pedido._id}>
            ...
            <div className="actions">
              ...
              <button
                className="btn-listo"
                onClick={() => cambiarEstado(pedido._id, "Listo para servir")}
              >
                Listo
              </button>
            </div>
